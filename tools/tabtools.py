...
import sys
import Levenshtein
def db_loader(target_ehr, dataset='mimic_iii'):
    ehr_mappings = {
        'mimic_iii': {
    ...
    data = pd.read_csv(ehr_mappings[dataset][target_ehr])
...
def sql_interpreter(command, dataset='mimic_iii'):
    db_path = f"data/{dataset}/{dataset}.db"
    con = sqlite3.connect(db_path)
    cur = con.cursor()
    results = cur.execute(command).fetchall()
    return results

def date_calculator(argument, dataset='mimic_iii'):
    try:
        db_path = f"data/{dataset}/{dataset}.db"
        con = sqlite3.connect(db_path)
        cur = con.cursor()
